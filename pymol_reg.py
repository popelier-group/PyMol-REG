...
def _create_reg_object(selection, reg_df, scale, color, n_terms, property):
    ...
    for i, val in reg_df.iloc[:n_terms].iterrows():
        ...
        atom2 = atom_index[1]
        cmd.select(
            name=f"temp_sele", selection=f"i. {atom1} i. {atom2}", domain=selection
        )
        cmd.distance(
            obj_name,
            selection1=f"{selection} like idx. {atom1} ",
            selection2=f" {selection} like idx. {atom2}",
        )
